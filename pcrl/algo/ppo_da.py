...
class PPO(Algorithm):
    ...
    def __init__(self, state_shape,  action_shape,  device, seed, gamma=0.995,
                 ...
                 epoch_ppo=10, clip_eps=0.2, lambd=0.97, coef_ent=0.01,          # coef_ent = 0.01
                 max_grad_norm=10.0):
    ...
    def step(self, env, state, t, info):
        t += 1
        ...
        if info:
            ...
            action, log_pi = self.explore1(state) # 要修改explore函数
            next_state, reward, done, reward_c = env.step(action)
...
class PPOExpert(PPO):
    ...
    def __init__(self, state_shape, action_shape,  device, path1, path2):
