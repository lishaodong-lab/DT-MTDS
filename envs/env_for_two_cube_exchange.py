...
class ur5Env_1(gym.Env):
    ...
    def reset(self):
        ...
        self.target_position = [0.5, 0.03, 0.0]
        p.resetSimulation()
        ...
        self.cube3 = p.loadURDF("cube_three.urdf", basePosition=selected_position)
        self.cube4 = p.loadURDF("cube_four.urdf", basePosition=[selected_position[0],selected_position[1],0.06])


        p.changeVisualShape(self.cube3, -1, rgbaColor = [0, 0, 1, 1])  
        p.changeVisualShape(self.cube4, -1, rgbaColor = [1, 1, 0, 1])   
    ...
    def step(self, action):
        ...
        self.cube3_distance1 = sqrt((self.position_cube3[0][0] - self.position_cube4[0][0]) ** 2 \
                                   + (self.position_cube3[0][1] - self.position_cube4[0][1]) ** 2)        
        self.cube3_distancez = sqrt((self.position_cube3[0][2] - self.position_cube4[0][2]) ** 2)
        ...
        if self.stage == 0:
            ...
            if self.cube3_distance <= 0.0125 and self.G == 2:
                self.move_gripper(self.position, self.postrue, list([0.01, 0.01]))
                self.G = 3
    ...
    def _reward(self):
        ...
        if terminated:
            ...
            if self.G == 3:
                self.reward = - self.cube3_distance1 - abs(self.cube3_distancez - 0.05)
            elif self.G == 2:
                self.stage = 2
                self.reward = -10
                self.done4 = True 
        ...
        if self.stage == 0:
            self.observation = np.array(
                [self.position[0][0] * 100, self.position[1][0] * 100, self.position[2][0] * 100,
                 ...
                 self.position_cube4[0][2] * 100, 0.0],
                dtype=float)
        elif self.stage == 1:
            self.observation = np.array(
                [self.position_cube4[0][0] * 100, self.position_cube4[0][1] * 100, self.position_cube4[0][2]  * 100,
